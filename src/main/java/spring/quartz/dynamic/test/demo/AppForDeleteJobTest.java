 ...
 *
 */
public class AppForDeleteJobTest 
{
    public static void main( String[] args ) throws SchedulerException, InterruptedException, ClassNotFoundException
    {
        ...
        List<JobModel> removeJonList = getRemoveJobListFromDB();
        for(JobModel job:removeJonList){
            if(job.getState().equals("0")){
                TriggerKey triggerKey = TriggerKey.triggerKey(job.getJobName(), job.getJobGroup());
                //会同时移除表里面存储的数据      qrtz_job_details、qrtz_triggers
                //schedule.unscheduleJob(triggerKey);
                schedule.deleteJob(JobKey.jobKey(job.getJobName(), job.getJobGroup()));
                System.out.println("delete job: " + triggerKey);
            }
        }
    ...
    private static List<JobModel> getRemoveJobListFromDB(){
        List<JobModel> jobList = getJobListFromDB();
        for(JobModel job:jobList){
            job.setState("0");
        }
